class MainController < UIViewController 
  ...
  def send_sms(sender)
      ...
      #else
        alert("device does not contain message composer")
      #end
        
      # message = MFMessageComposeViewController.alloc.init.autorelease
      #       if MFMessageComposeViewController.canSendText
      #         message.body = "My name is #{table_data[0].fullName},\nMy age is #{table_data[0].age},\n i am #{table_data[0].gender},\nand i want to send my following Message: #{table_data[0].message},\nMy Phone Status:,\nBattery charge level: #{(batLeft*100).round.abs.to_s}%,\nMy Gps coordinates:\nLatitude: #{@latitude},\nLongitude: #{@longitude}\nand google map link https://maps.google.com/maps?q=#{@latitude},#{@longitude}"
      #         message.recipients = [table_data[0].phone_number1, table_data[0].phone_number2, table_data[0].phone_number3, table_data[0].phone_number4, table_data[0].phone_number5]
