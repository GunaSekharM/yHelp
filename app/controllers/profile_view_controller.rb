class ProfileViewController < UIViewController
  ...
  def add_phone
    if rmq(:phone_number_text).get.text.length != 10
      alert("please enter valid phone number")
    elsif @phone_numbers.count < 5
  ...
  def upload
    if rmq(:full_name_text).get.text != "" && rmq(:age_text).get.text != "" && rmq(:gender_text).get.text != "" && rmq(:sos_text).get.text != "" && @phone_numbers.count == 5
